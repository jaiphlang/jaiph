...
jaiph use 0.15.0       # reinstalls the v0.15.0 release binary
```
...
The command prints `jaiph <version>`, taken from the installed release at build time. After `jaiph use <version>`, run `jaiph --version` again and confirm the printed version matches. For example, you should see `jaiph 0.15.0` after `jaiph use 0.15.0`.
...
Pin both the action ref (`@v0.15.0`) and the `version` input to an exact release for reproducible CI. Use `nightly` to track the rolling prerelease. The action supports GitHub-hosted Linux and macOS runners on arm64 and x64, which are the platforms that have release artifacts.
