...
- **Send operator** — `channel <- echo "data"` sends content to a named inbox channel; the channel identifier is always on the left side of `<-`. Standalone `channel <-` forwards `$1`. The runtime dispatches to workflows registered via route declarations. See [Inbox & Dispatch](inbox.md).

- **Route declaration** — `channel -> workflow` registers a static routing rule: when a message arrives on `channel`, call `workflow` with the message as `$1`. Multiple targets (`ch -> wf1, wf2`) dispatch sequentially. Routes are declarations, not executable steps. See [Inbox & Dispatch](inbox.md).
