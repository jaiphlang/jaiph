...
2. **Spawn-env and journal guardrails, not a sandbox.** A host key reaches a script only when the script's declaration requests it with a [`use` clause](script-env.md) *and* the operator grants it with `--env`. `--env` values are injected only into the subprocesses whose declaration `use`s the key; they are not present on the runner (workflow-leader) process environment, which Jaiph builds from an allowlist (process basics, `JAIPH_*` control keys, backend credentials) rather than a copy of your shell — an ungranted host key is absent from it. A fail-closed scrub builds each `prompt` child's environment the same way: anonymous prompts never receive `--env` keys; a named prompt receives only keys it `use`s. Credential-shaped values are redacted from the run journal and from returned call diagnostics. Those rules stop accidental inherit, a script printing a token into the journal, and interpolation of a key that was never meant for the model. They do not isolate the agent: this is not a sandbox — a same-user agent that can run a shell can still reach a key on disk or in another process, so it is not confined to child `env`. A `cursor` or `claude` prompt that can run tools runs as the same user as `jaiph` and can open the workspace, `$HOME`, and other processes that user owns. The `codex` backend talks to an HTTP endpoint and runs no local program, so that agent has no shell on the host machine. Isolation of the whole run is an outer concern — a container, pod, or a dedicated user you create, not an install-time account. See [Deploy jaiph](deploy.md).
