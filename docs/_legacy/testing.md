...
In **discovery mode** (no path, or a directory path), zero matches print `jaiph test: no *.test.jh files found (nothing to do)` to stderr and exit **0** — so `jaiph test` is safe to call unconditionally from CI and agent loops. Passing an explicit file path that does not exist or is not a `*.test.jh` file still exits **1** — a named target must exist. A file must contain at least one `test` block; otherwise the CLI reports a parse error. Passing a plain `*.jh` file that is not named `*.test.jh` is rejected — use `jaiph run` for those.
...
| `JAIPH_PROMPT_RETRY` | Defaulted to `0` by the test runner so a failing prompt (e.g. no matching mock arm) surfaces immediately instead of waiting through the production retry backoff (`15s → 1m → 10m → 30m → 2h`). If the parent environment already sets `JAIPH_PROMPT_RETRY`, the harness honors it — so a test that intentionally exercises the retry loop can opt in by exporting the variable. See [Configuration — Prompt retry on transport failure](configuration.md#prompt-retry-on-transport-failure). |
