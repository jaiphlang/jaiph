...
- **Snapshot mode (default)** — before launching, the CLI takes a **writable point-in-time snapshot** of the workspace (a host-side clone, block-level copy-on-write where the filesystem supports it) and bind-mounts that snapshot read-write at `/jaiph/workspace`. The snapshot content is **git-defined** — tracked and untracked-but-not-ignored files plus `.git/`, with gitignored paths absent (see [What the snapshot contains](#snapshot-content)). The live host workspace is **never mounted into the container at all**: host changes during the run are invisible to the container, and the container's workspace writes are discarded when the snapshot is deleted at exit. The snapshot lives at `<run dir>/sandbox` (under `.jaiph/runs/` by default) and is masked from the container's own `/jaiph/run` view by a tmpfs so the run cannot read its snapshot source back.
...
The snapshot is **git-defined**, not a raw copy of the directory. For a git workspace, the snapshot contains exactly:

- every file git reports from `git ls-files --cached --others --exclude-standard` — tracked files plus untracked files that are **not** gitignored, and
- the `.git/` directory wholesale, so workflows can read history and commit inside the sandbox.

Nothing else. **Gitignored files never enter the sandbox.** A `.env`, a `credentials.json`, an `.npmrc` with a token, a built `dist/`, and a `node_modules/` tree are all absent from `/jaiph/workspace` — not empty, *absent*. This is the filesystem twin of the [environment allowlist](#env-exposure): the sanctioned way to get a secret into a run is explicit injection into trusted steps ([`--env`](cli.md#jaiph-run) or [`trusted_envs`](configuration.md#trusted-envs)), never "it happened to be lying in a gitignored file". It is also the dominant clone-speed win — ignored artifact directories are usually the overwhelming majority of a workspace's file count, and they are never even scanned.

The content is **identical regardless of which copy mechanism runs** (APFS clonefile, block-level reflink, or a plain data copy) and regardless of platform — what the agent sees is a property of git's answer, not of the host filesystem.

Details and edges:

- **git is the only gitignore oracle.** Jaiph consumes git's file list; it does not reimplement ignore semantics (nested `.gitignore` files, `!` negations, `.git/info/exclude`, global excludes). Whatever git says is in the tree is what the sandbox gets — exactly.
- **Non-git workspace** (no `.git` at the workspace root, or `git ls-files` fails): Jaiph falls back to copying **everything** (minus the run-artifacts directory). With no git to consult, an "ignored-looking" file such as `.env` *is* copied. If you rely on the git-defined exclusion, run inside a git repository.
- **Submodules** are copied wholesale. A `.gitmodules`-registered path appears as a single gitlink entry in `git ls-files`; Jaiph copies that directory as an opaque subtree.
- **Tracked-but-deleted files** (listed by `ls-files --cached` but removed from the worktree and not yet committed) are simply skipped — they are not on disk to copy.

**Consequence — the sandbox is a clean checkout plus untracked files.** Because `node_modules/` (and other gitignored build output) is absent, a workflow that builds or tests **must install its dependencies inside the container** — `npm install`, `pip install`, `cargo fetch`, etc. — exactly as a fresh CI checkout would. There is no config escape hatch to re-include ignored paths in this model; if a workflow needs a dependency tree, it installs it as one of its steps.
...
- **Filesystem reach** — scripts inside the container cannot read or write arbitrary host paths outside the workspace mount and the run-artifacts mount. The rest of the host is invisible to the container. In the default snapshot mode the container works on a point-in-time clone, so the live host workspace is never even mounted and is unmodified after the run. That clone is [git-defined](#snapshot-content): gitignored files (secrets in `.env`, tokens in `.npmrc`, …) never enter the container in the first place.
