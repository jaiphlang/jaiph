...
- **Filesystem access** -- Scripts inside the container cannot read or write arbitrary host paths *outside* the workspace, regardless of mode. The container's `/jaiph/workspace` is one of: an in-container `fuse-overlayfs` merge over a read-only bind of the host workspace (overlay mode; writes go to the overlay upper/work dirs under `/tmp` inside the container and are discarded on exit); a host-side clone of the workspace mounted read-write (copy mode; the clone is removed on exit unless kept for debugging); or — for the explicit opt-in **inplace mode** — the real host workspace bind-mounted read-write so the run's edits land live on the host (see [Inplace mode](#inplace-mode-trusted-workspace-untrusted-machine)). Outside the workspace mount and the run-artifacts mount (`/jaiph/run`), the rest of the machine stays sandboxed in every mode.
- **Process isolation** -- Container processes cannot see or signal host processes. Every sandboxed container uses `--cap-drop ALL` plus `--security-opt no-new-privileges`. **Overlay mode** (Linux) adds capabilities required for `fuse-overlayfs` and for dropping privileges after mount: `SYS_ADMIN`, `SETUID`, `SETGID`, `CHOWN`, and `DAC_READ_SEARCH` (see `buildDockerArgs` in `src/runtime/docker.ts`). **Copy mode** and **inplace mode** do not add capabilities. The overlay entrypoint (`runtime/overlay-run.sh`) starts as the container user `0:0` so it can mount, then normally **`exec`s `jaiph run` as the host UID/GID** via `setpriv` when `JAIPH_HOST_UID` / `JAIPH_HOST_GID` are set; copy and inplace modes use `--user <host_uid>:<host_gid>` directly so files written to the workspace land owned by the host user, not root. macOS Docker Desktop does not use Linux `--user` overrides (UID mapping is handled by the VM).
...
- **Workspace mutation when `inplace` is opted into.** Setting `JAIPH_INPLACE=true` is an explicit "trusted workspace, untrusted machine" posture: the host workspace is bind-mounted read-write so the run's edits persist live on the host. The machine outside the workspace stays sandboxed as in any mode, but a crashed or malicious run can mutate files in your real checkout — see [Inplace mode](#inplace-mode-trusted-workspace-untrusted-machine).
...
By default the container's `/jaiph/workspace` *looks* writable to scripts but never mutates the host checkout. The CLI picks one of three sandbox primitives at launch time:
...
- **Inplace mode** (selected only when `JAIPH_INPLACE=1` or `JAIPH_INPLACE=true` is set explicitly — never a default). The host workspace itself is bind-mounted read-write at `/jaiph/workspace`, so the run's edits land live on the host. The machine boundary (mount allowlist, dropped capabilities, `no-new-privileges`, env allowlist) is identical to copy mode — only the workspace isolation is removed. This mode trades the "host checkout untouched" guarantee for a real iterate-on-real-files dev loop, and triggers an interactive confirmation prompt before launch — see [Inplace mode](#inplace-mode-trusted-workspace-untrusted-machine).

The copy-mode clone copies each top-level entry of the workspace; under `.jaiph` it copies everything except the `runs` tree (run logs live under the separate `/jaiph/run` mount). **`.git` is not stripped** — workflows that call `git` see the same history as on the host, at the cost of a larger copy in copy mode on Linux. Overlay mode and inplace mode mount the real workspace and so always see the live `.git`.

In all modes, run artifacts are written to a separate rw mount at `/jaiph/run` (outside the workspace sandbox) so they persist to the host.
...
### Inplace mode (trusted workspace, untrusted machine)
{: #inplace-mode-trusted-workspace-untrusted-machine}

**Inplace mode** keeps the sandbox **on** and lets the run **edit your real workspace live**. It is a different axis from `JAIPH_UNSAFE`: `JAIPH_UNSAFE=true` turns the sandbox *off* (workflow runs on the host with no container at all), whereas `JAIPH_INPLACE=true` keeps the container, the dropped capabilities, `no-new-privileges`, and the env allowlist in place, and only removes the workspace-isolation half of the protection. The posture is **"trusted workspace, untrusted machine"**: you accept that the run will mutate files inside the workspace directly, in exchange for getting persistent edits without the copy-mode clone or the overlay discard.

Typical use case: an agent-driven dev loop where you want the model's edits to land directly in your checkout so you can `git diff`, run your editor, and iterate without a manual patch-export step.
...
1. The CLI bind-mounts the host workspace at `${CONTAINER_WORKSPACE}:rw` (no clone, no read-only layer, no `overlay-run.sh`, no `/dev/fuse`, none of the overlay-only `--cap-add` flags). The runs mount (`/jaiph/run:rw`) and the capability/security posture (`--cap-drop ALL`, `--security-opt no-new-privileges`) are identical to copy mode.
2. The CLI shows an interactive **warning + confirmation prompt** (see below) before launching the container.
3. On Linux the container runs as `--user <host_uid>:<host_gid>` (same as copy mode) so files the run creates are owned by you, not root.

**Destructive-edit safeguard.** Because a crashed or killed run now leaves your real workspace half-mutated with no rollback, the CLI **must** confirm before launch. The warning is tailored to your git state and always names the actual workspace directory:

- **Clean git tree:** a successful or failed run can be undone with `git restore .` (or `git reset --hard`).
- **Dirty git tree:** the run's edits will be mixed in with your uncommitted changes and cannot be cleanly undone — the prompt suggests committing or stashing first.
- **No git repo** (or `git` not on PATH): there is no safety net; the prompt names this as irreversible and suggests `git init` and a baseline commit.
...
**Env-var leak protection.** `JAIPH_INPLACE` and `JAIPH_INPLACE_YES` would otherwise pass the `JAIPH_*` allowlist for env forwarding (`JAIPH_DOCKER_*` is the only built-in exclusion prefix). The CLI explicitly excludes both names so they are **not** forwarded into the container — this prevents a nested `jaiph run` inside the workflow from re-triggering inplace mode on its own and keeps host control flags out of script environments. See [Environment variable forwarding](#environment-variable-forwarding).

**Run banner.** The banner reports inplace mode distinctly from overlay and copy: the parenthetical reads `Docker sandbox, in-place (live host edits)` (compared to `Docker sandbox, fusefs` for overlay or `Docker sandbox, tmp workspace` for copy). See [CLI — Banner](cli.md#jaiph-run).

**macOS performance note.** On macOS the `:rw` bind-mount goes through Docker Desktop's virtiofs file-sharing layer, so write throughput is slower than the APFS `cp -cR` clone that copy mode uses. This is acceptable for the dev-loop use case but worth knowing if you are processing many small files.

**Known sharp edges.**

- **Concurrent runs.** Two `jaiph run` invocations against the same workspace in inplace mode will write into the same directory simultaneously. The CLI does not lock or serialize parallel runs — avoid this manually until locking is added.
- **No rollback on crash/timeout.** If the container is killed (timeout, SIGTERM, SIGKILL, host crash), partial edits remain on disk. The git-state confirmation above is your only safety net.
...
| `runtime.docker_timeout_seconds` | integer | `14400` | Max execution time in seconds (default four hours). Must be a non-negative integer; `0` disables the timeout. Negative values produce `E_DOCKER_TIMEOUT`. |
...
**Workspace immutability contract** -- In the **default** modes (overlay and copy), Docker runs cannot directly modify the host workspace. In overlay mode the host checkout is bind-mounted read-only and writes land in the overlay upper layer (under `/tmp` inside the container) and are discarded on container exit. In copy mode the container writes to a separate host-side clone of the workspace (`<runs-root>/.sandbox-<id>/`), which is removed on container exit unless explicitly kept for debugging. In both modes the only persistence channel from a Docker run to the host is the run-artifacts directory (`/jaiph/run` → host `.jaiph/runs`). **Inplace mode (`JAIPH_INPLACE=true`) is the explicit opt-out from this contract** — the host workspace itself is bind-mounted rw and all writes persist live on the host; see [Inplace mode](#inplace-mode-trusted-workspace-untrusted-machine). Non-Docker (local) runs are unaffected by this contract.
...
**Timeout** -- When the effective timeout (from `JAIPH_DOCKER_TIMEOUT` or `runtime.docker_timeout_seconds`, after the merge in [Configuration keys](#configuration-keys)) is greater than zero, the CLI arms a timer on the spawned `docker` child; on overrun it sends `SIGTERM`, then `SIGKILL` after a 5-second grace period. The failure message includes `E_TIMEOUT container execution exceeded <N>s — increase runtime.docker_timeout_seconds or JAIPH_DOCKER_TIMEOUT`, where `<N>` is the configured timeout in seconds. `0` disables the timer.

> **Interaction with prompt-retry backoff.** A full default prompt-retry run can wait up to ~2h41m of wall-clock for transport failures (`15s → 1m → 10m → 30m → 2h` across six attempts; see [Configuration — Prompt retry on transport failure](configuration.md#prompt-retry-on-transport-failure)). In a Docker sandbox this is capped by the container timeout — the default 4h budget covers one full backoff but leaves little headroom for the workflow's other work. For workflows that need to retry multiple flaky prompts inside a container, either raise `runtime.docker_timeout_seconds` / `JAIPH_DOCKER_TIMEOUT`, shorten the schedule with `JAIPH_PROMPT_RETRY_DELAYS`, or disable retries with `JAIPH_PROMPT_RETRY=0`.
...
- `JAIPH_*` (except `JAIPH_DOCKER_*`, excluded to prevent nested Docker execution; and except the explicit names `JAIPH_INPLACE` and `JAIPH_INPLACE_YES`, which are host-only switches — see below)
- `ANTHROPIC_*`
- `CURSOR_*`
- `CLAUDE_*`

**Explicit name exclusions.** Two `JAIPH_`-prefixed variables would otherwise pass the allowlist but are blocked from forwarding by name: `JAIPH_INPLACE` and `JAIPH_INPLACE_YES`. Both are host CLI flags that select inplace mode (see [Inplace mode](#inplace-mode-trusted-workspace-untrusted-machine)); forwarding them would let a nested `jaiph run` inside the workflow re-trigger the same mode against the in-container workspace and would expose the host's automation-confirm posture to script code. The exclusion is enforced in `isEnvAllowed`.
