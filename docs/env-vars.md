...
| Variable | Type | Default | Role |
|---|---|---|---|
| `JAIPH_REPO_REF` | string | `v0.14.0` (installer default when unset) | Release ref the installer downloads (`v0.14.0`, `nightly`, …). `jaiph use <version>` sets this to `v<version>` or `nightly`. |
