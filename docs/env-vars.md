...
| `JAIPH_SERVE_EXPOSE_DOCS` | host | bool | `true` | — | `jaiph serve` — expose `GET /docs` (Swagger UI) and `GET /openapi.json`. Set `false` (or `0`) to return `404` for both so a hardened deployment hides its API surface. `/healthz` is always available and credential-free. |
