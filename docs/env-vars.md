...
| `OPENAI_API_KEY` | `codex` | hard error (`E_AGENT_CREDENTIALS`) | hard error (`E_AGENT_CREDENTIALS`) | No CLI-login fallback. `OPENAI_*` is on the Docker forwarding allowlist — set on the host before `jaiph run`. |

Forwarding allowlist prefixes into the Docker container: `JAIPH_*` (except `JAIPH_DOCKER_*`, `JAIPH_INPLACE`, and `JAIPH_INPLACE_YES`), `ANTHROPIC_*`, `CLAUDE_*`, `CURSOR_*`, `OPENAI_*`. Everything else — including unrelated cloud credentials — is silently dropped — see [Sandboxing](sandboxing.md).
