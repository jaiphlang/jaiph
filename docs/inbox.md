...
- **Late binding.** A def produces something, such as a finding, a summary, or a verdict, and another def should react to it. The producer should not have to know the consumer's name or its parameter list.
- **Fan-out to several listeners.** Several defs should react to the same event. Running a separate message broker for an in-process runner is heavy, and it forces you to run and manage another service just to pass messages between defs in the same process.

Channels give defs a way to publish and subscribe without leaving the process. The producer sends a message on a channel with `send`. The channel declaration lists one or more defs that listen on it, and the runtime delivers the message to each of them.

## When messages are delivered

A `send` does not run the route targets at the moment the line executes. Instead, each running def has a frame with its own in-memory queue. When the runtime runs a `send`, it adds the message to a queue and drains the queue later.
...
A short round trip looks like this. The producer sends a payload on `findings`, and the runtime delivers it to `analyst` after `main` finishes its steps:

```jh
channel findings -> analyst

export def main() {
  send "found a null deref in parser.ts" -> findings
}

def analyst(message) {
  log "analyst got: ${message}"
}
```

Routes are top-level data on `ChannelDef`, not statements inside a def body. Declaring routes on the channel has the following effects:
...
- When a target fails, the runtime stops the whole drain pass. It skips the remaining targets for that message and dispatches no more messages. The runtime reports the failure as the failure of the def that owns the queue. There is no per-target error handling, so the first failure stops the whole pass (fail-fast).
...
A `send` behaves in one of two ways, depending on whether any running def declares a route for the channel:

- **Routed.** Some running def has a route for the channel under its bare name. If the channel is written with an imported `alias.` prefix, the runtime strips the prefix before it looks up the route. The runtime walks outward from the sender until it finds that def's frame and adds the message to that frame's queue. It also writes an audit copy of the message to `inbox/NNN-<channel>.txt` under the run directory.
- **Unrouted.** No running def has a route for the channel. The runtime still adds the message to the sender's own queue and still writes an `INBOX_ENQUEUE` event to `run_summary.jsonl`. It does not write an audit file, and the sender's drain pass skips the message because there are no targets to run.

An unrouted send is dropped on purpose, and it is not an error. Subscribers can be optional this way. A def can send on `metrics` even when nothing listens today, and you can add a subscriber later without changing the producer. If a missing subscriber should be a hard failure, check for it in a test or in a `def`, not in the channel runtime.

## How a receiver def is called

A receiver is a normal def. The runtime binds a prefix of up to three positional arguments:
...
The receiver chooses its own parameter names and can declare one, two, or three parameters. The runtime passes only as many arguments as the receiver declares.

## Summary

Channels are a deliberately small feature. A channel passes a message from one def to one or more other defs, inside the same process. You declare the routes once at the top of the module, and the compiler validates them. Delivery happens when a step list finishes successfully, and the runtime dispatches the targets one at a time. The runtime records every send and every delivery in `run_summary.jsonl`, and it writes an `inbox/` audit file for each routed send, so you can inspect what happened after the run. Channels leave out concurrency, message brokers, retries, and dead-letter queues on purpose. Those problems belong to other tools, and leaving them out keeps channels small enough to understand without going outside the runtime.
...
- [Spec: Async Handles](spec-async-handles.md) describes the implicit join that runs before a def's queue drains.
- [Language](language.md#send-channel-message) and [Grammar](grammar.md) cover the `channel` and `send` syntax.
