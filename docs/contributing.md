...
**`JAIPH_E2E_SKIP_DOCKER=1`** (set by `.jaiph/ensure_ci_passes.jh` for overnight engineer loops) makes **`e2e/test_all.sh`** skip scripts whose basename matches `*docker_*` / `*_docker` / `docker_*`, so load-flaky Docker Desktop e2e cannot trap the recover agent. GitHub Actions does **not** set this — full Docker e2e remains required there. Static Dockerfile checks such as **`09_dockerfile_fetch_verify`** still run. Hardening those scripts so the skip can be removed is queued in **`QUEUE.md`**.
