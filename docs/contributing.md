...
| Job | Runner | Purpose |
|-----|--------|---------|
| **Compiler and unit tests** | `ubuntu-latest` | `npm run arch:check` (the dependency-cruiser import-graph gate for no cycles and the layer DAG) then `npm test` (TypeScript unit + acceptance + golden tests), plus a `curl` check that the public install URL responds and a git-tag verification on `main`. |
