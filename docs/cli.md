...
| Flag | Argument | Effect |
|---|---|---|
| `--host` | `<addr>` | Listen address (default `127.0.0.1`). Binding with no authentication (neither `JAIPH_SERVE_TOKEN` nor OIDC) aborts startup unless `--allow-anonymous` is passed. |
