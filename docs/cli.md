...
| `<version>` (e.g. `0.15.0`) | Reinstalls the release binary for tag `v<version>`. |
...
| Flag | Argument | Effect |
|---|---|---|
| `--host` | `<addr>` | Listen address (default `127.0.0.1`). A non-loopback bind with no `JAIPH_SERVE_TOKEN` and no OIDC aborts startup unless `--allow-anonymous` is passed. |
