...
- **`catch` is a one-shot handler.** If the target fails, the catch body runs once. If the catch body succeeds, the join counts the async branch as successful. If the catch body ends with a `return`, that value becomes the branch's contribution to a parent workflow return.
