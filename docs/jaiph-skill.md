...
**Jaiph** is a small language for describing workflows: **orchestration steps** (rules, prompts, managed calls) and **bash in `script` blocks**, compiled to Bash plus a runtime that records steps, routes messages between workflows, and (for rules) applies read-only isolation where the platform supports it. **`jaiph run`** / **`jaiph test`** **launch** workflows from the **CLI** (TypeScript), which executes the generated workflow module directly in `__jaiph_workflow` mode (no `bash -c` wrapper) and provides metadata pathing through `JAIPH_META_FILE`; generated bash **`source`s `jaiph_stdlib.sh`**, which in turn calls the **bundled JS kernel** (`src/runtime/kernel/` — **prompt**, **managed step subprocess** execution, **file-backed inbox**, **stderr / summary events**) via **`node`**. User **`script`** bodies stay separate OS processes. **`.jaiph.map`** files map **`.sh`** lines to **`.jh`** for stderr diagnostics.
