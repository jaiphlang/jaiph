...
- **Rules** — Structured checks: `ensure` (other **rules** only), `run` (**scripts** only — not workflows), `const`, `match`, `fail`, `log`/`logerr`, `return "…"` / `return run script()` / `return ensure rule()`, `ensure … recover`, `run … recover`. No raw shell lines, `prompt`, inbox send/route, `wait`, or `run async`. Under `jaiph run`, rule bodies are executed **in-process** by the Node runtime; when a rule runs a **script**, that script is a normal managed subprocess (same as scripts from workflows) — see [Sandboxing](sandboxing.md).
- **Workflows** — Named sequences of **Jaiph-only** steps: `ensure`, `run`, `prompt`, `const`, `fail`, `return`, `log`/`logerr`, inbox **send** (`channel_ref <- …`), `match`, `run async`, `ensure … recover`, `run … recover`, and `wait` (a no-op in the Node runtime — useful as a placeholder; **not** allowed in rules). Route declarations (`->`) belong at the top level on `channel` declarations, **not** inside workflow bodies — a `->` inside a body is a parse error. Unrecognized lines are errors — put bash in **`script`** definitions and call with `run`.
...
- **ensure** — Runs a rule; succeeds if exit code is 0. Optional `recover` handles failure: the recovery body runs **once** (like a catch clause) — there is no retry loop. `recover` requires explicit bindings: `recover (<name>) { … }` or `recover (<name>, <attempt>) { … }`. The first binding receives merged stdout+stderr from the failed rule; the optional second binding receives the attempt number (always `"1"`). Bare `recover` or `recover {` without bindings is a parse error. For retries, use explicit recursion in the recovery body. `ensure … recover` works in both workflows and rules.
- **run … recover** — Same semantics as `ensure … recover`, but for `run` call sites (workflows and scripts). `run ref() recover (err) { … }` catches failures from the target and runs the recovery body once. Works in both workflows and rules.
...
- **Error handling:** Use `recover` for failure handling and `match` for value branching. The `if` keyword is removed from the language (produces `E_PARSE`).
  - `ensure ref() recover (err) { … }` — recovery on rule failure.
  - `run ref() recover (err) { … }` — recovery on script/workflow failure.
  - `match var { “pattern” => …, _ => … }` — value branching on string values.
  - Express “shell conditions” with `run` to a **script** and handle failure with `recover`. Short-circuit brace groups remain valid **inside `script`** bodies: `cmd || { ... }`.
...
- Inside a rule, use `ensure` for **rules** and `run` for **scripts only** — not `prompt`, `send`, `wait`, or `run async`.
