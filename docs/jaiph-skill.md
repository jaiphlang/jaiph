...
- **Steps:** `ensure ref [args...]`, optionally `ensure ref [args] recover <body>`. For ensure, `ref` is a rule name or `alias.rule_name`; args are passed to the shell. With `recover`, the step is a **bounded retry loop**: run the rule; on failure run the recover body (single statement or `recover { stmt; stmt; ... }`); repeat until the rule passes or max retries (default 10, override via `JAIPH_ENSURE_MAX_RETRIES`) is exceeded, then exit 1. `run ref [args...]` runs a workflow; args are passed to the workflow. **Assignment capture:** `name = ensure ref`, `name = run ref`, or `name = <shell_command>` captures that step's stdout into `name` (e.g. `response = ensure tests_pass`). Same exit semantics as without capture: failure fails the step unless you add `|| true`; stderr is not captured unless the command redirects it (e.g. `2>&1`). See [Grammar](grammar.md).
