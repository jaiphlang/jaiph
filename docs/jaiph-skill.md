...
- **Steps:**
  ...
  - **run** — `run ref([args...])` runs a workflow or script (local or `alias.name`); explicit args are comma-separated inside parentheses. **`run` does not forward args by default** — pass positional args explicitly (e.g. `run wf("${arg1}")`, `run util_fn("${arg2}")`). **Bare identifier arguments** are supported: `run greet(name)` is equivalent to `run greet("${name}")` — the identifier must reference a known variable (`const`, capture, or `arg1`–`arg9`); unknown names fail with `E_VALIDATE`. Jaiph keywords cannot be used as bare identifiers. Also supports **inline scripts**: `run script("body" [, "arg1", ...])` — see Scripts section above.
