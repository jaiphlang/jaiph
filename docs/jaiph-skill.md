...
**Jaiph** is a small language for describing workflows: **orchestration steps** (rules, prompts, managed calls) and **bash in `script` blocks**, compiled to Bash plus a runtime that records steps, routes messages between workflows, and (for rules) applies read-only isolation where the platform supports it. **`jaiph run`** / **`jaiph test`** **launch** workflows from the **CLI** (TypeScript), which spawns bash to **`source`** the generated module; generated bash **`source`s `jaiph_stdlib.sh`**, which in turn calls the **bundled JS kernel** (`src/runtime/kernel/` — **prompt**, **managed step subprocess** execution, **file-backed inbox**, **stderr / summary events**) via **`node`**. User **`script`** bodies stay separate OS processes. **`.jaiph.map`** files map **`.sh`** lines to **`.jh`** for stderr diagnostics.
