...
- **Arguments arrive as `$1`, `$2`, …** Module `const` values and def bindings are *not* exported into the subprocess environment — pass them explicitly as arguments. For a large string (argv is bounded by `ARG_MAX`, ~1 MB), pipe it to the script's stdin instead: `run save_string_to_file(path) stdin content` (script-only; `E_PARSE` on `run async`). See [Language — Arguments and stdin](language.md#arguments-and-stdin).
...
- **Block-scoped, not hoisted.** A nested name is visible only after its declaration and only inside the body that declares it, which is the def body or an `if` / `else` / `for` / `catch` / `recover` block. Using it earlier, or after the branch ends, is `E_VALIDATE`. A nested `def` may `run` **itself** (self-recursion, bounded by the depth limit of 256), but not a sibling nested `def` declared later in the same body (still `E_VALIDATE`, not hoisted). **No `export`** on a nested declaration (`E_PARSE`); the nested forms are `const` / `script` / `def` / named `prompt` only. Inside a nested def, `import` and a `config { … }` block are also `E_PARSE`.
