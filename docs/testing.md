...
The project includes a Playwright-based test (`tests/e2e-samples/landing-page.spec.ts`) that verifies landing-page code samples stay in sync with real CLI behavior. It serves the Jekyll docs site locally, extracts sample source and expected output from stable `data-sample` DOM attributes in `docs/index.html`, then compares source against `examples/*.jh` files and runs deterministic samples through the CLI. Run it with `npm run test:samples`. See [Contributing — Landing-page sample verification](contributing.md#landing-page-sample-verification-playwright) for setup and details.
