...
- **Scripts** — Top-level **`script`** blocks: plain bash with transpiler ref-resolution (still emitted as bash functions under the hood). From a **workflow** or **rule**, call them only with **`run`**. Bodies may not use Jaiph keywords `fail`, `const`, `log`, `logerr`, or Jaiph-style `return "…"` / `return "$var"` — use bash `return N` / `return $?` and **stdout** (`echo`, `printf`) for data passed back to `run` callers. Scripts cannot be used with `ensure`, are not valid route targets, and cannot be exported.
...
script_decl     = "script" IDENT [ "()" ] "{" { script_line } "}" ;
...
**Script bodies** are bash only: they must not contain `run`, `ensure`, `config`, nested top-level declarations, inbox route lines (`channel -> ...`), or Jaiph keywords **`fail`**, **`const`**, **`log`**, **`logerr`**, or Jaiph-style **`return "…"`** / **`return "$var"`**. Optional shared libraries (`.sh` under `.jaiph/lib/`) load with `source "$JAIPH_LIB/…"`. Generated scripts default `JAIPH_LIB` to `${JAIPH_WORKSPACE:-.}/.jaiph/lib`; the runtime sets `JAIPH_LIB` when executing **script** steps.
...
6. **Scripts:** Each top-level script is emitted as `<module>::<name>::impl`, `<module>::<name>` (wrapper using `jaiph::run_step` / the same managed-step machinery as workflows, with step kind `script`), and a shim `<name>` that forwards to the namespaced wrapper for non-workflow shell callers. Workflow steps invoke scripts via `run`, which records the call like any other managed step.
