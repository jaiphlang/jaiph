...
8. **Conditional steps, fail, wait, prompt schema, recover, capture:** Brace `if` lowers to bash `if …; then … fi`. **`fail`** → stderr message + `exit 1`. **`wait`** → bash `wait`. **Prompt + `returns`:** `jaiph::prompt_capture_with_schema` with JSON extraction and schema validation (same exit codes as before). **ensure … recover:** bounded retry loop; failed rule’s explicit **`return`** value as `$1` in the recover body. **Assignment capture:** **Rules and workflows** use `JAIPH_RETURN_VALUE_FILE` and **`jaiph::set_return_value`** for **`return "…"`** / **`return "$var"`**. **Scripts** do **not** emit `jaiph::set_return_value`; **`run` to a script** captures **stdout** instead.
