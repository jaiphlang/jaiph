...
Jaiph source files (`.jh`) combine a small **orchestration** language with Bash inside **`script`** blocks. The compiler parses top-level declarations and structured steps in workflows and rules, validates references and bash fragments (where they still appear), then emits a Bash script that relies on the Jaiph stdlib for managed steps, agents, logging, and inbox routing, plus a **`.jaiph.map`** sidecar so **`.sh`** line numbers map back to **`.jh`** for CLI diagnostics. **`jaiph run`** / **`jaiph test`** start from the **TypeScript CLI**: it compiles, then spawns a **bash** child that **`source`s** the module (see [CLI — `jaiph run`](cli.md#jaiph-run)); **prompt** execution, **managed step subprocesses**, and **stderr `__JAIPH_EVENT__` / `run_summary.jsonl` lines** are implemented in the **JS kernel** (`runtime/kernel/*.js`, invoked from the stdlib). Step sequencing and bash-visible control flow stay in generated bash + **`jaiph_stdlib.sh`**; there is **no supported alternate** where workflows are driven end-to-end by bash alone without that kernel-backed stack. This page is the language reference: what you can write, what it means at runtime, and how it maps to generated shell.
