...
const_rhs       = quoted_or_multiline_string | single_quoted_string | bash_value_expr
  ...
  (*   E_PARSE with guidance, e.g. Function/script calls in const assignments must use run. *)
...
**Script bodies** are bash only (unless a custom shebang is provided): they must not contain `run`, `ensure`, `config`, nested top-level declarations, inbox route lines (`channel -> ...`), or Jaiph keywords **`fail`**, **`const`**, **`log`**, **`logerr`**, or Jaiph-style **`return "…"`** / **`return "$var"`**. For **custom shebang** scripts (e.g. `#!/usr/bin/env node`), Jaiph keyword validation is skipped — the body is treated as opaque content for the target interpreter. Optional shared libraries (`.sh` under `.jaiph/lib/`) load with `source "$JAIPH_LIB/…"`. Generated scripts default `JAIPH_LIB` to `${JAIPH_WORKSPACE:-.}/.jaiph/lib`; the runtime sets `JAIPH_LIB` when executing **script** steps.
...
2. **ensure:** An optional argument tail after the REF is passed through to the shell (e.g. `ensure check_branch "$1"`). With `recover`, the step becomes a retry loop: run the condition; on failure run the recover body; repeat until the condition passes or max retries are reached. Recover is either a single statement (`ensure dep recover run install_deps`) or a block of statements separated by `;` or newline (`ensure dep recover { run a; run b }`). The recover body sees **`$1`** from the ensure-retry capture channel (`JAIPH_RETURN_VALUE_FILE`) for the failed attempt. For script-driven failures, runtime capture includes script stdout written during that attempt (including nested scripts; stderr only when merged into stdout, e.g. `exec 2>&1` in the script). Step artifacts under `.jaiph/runs/` remain the full record. If none of that applies, **`$1`** may be empty — persist or assert before prompting if your recover text promises a log file on disk.
...
8. **`const` declarations:** `const name = <rhs>` introduces an immutable binding in the workflow (or rule) body. RHS forms: same **value** expressions allowed for module `const` (no `$(...)` or disallowed `${...}` ops), or explicit **`run` / `ensure` / `prompt`** capture (workflow only for `prompt`). **Call-like** text — a **reference** that resolves to a rule, workflow, or script, followed by **arguments** — is **not** a valid bare RHS: write **`const name = run ref [args…]`** (or **`const name = ensure ref [args…]`** for rule capture), not **`const name = ref [args…]`**. A bare **`ref args…`** would not lower to a managed step and previously risked broken assignments in generated shell; the compiler rejects it with **`E_PARSE`** and a line showing the **`run`** form. Transpilation uses a bash `local` for the name and assigns from the value or managed capture, same as `name = run|ensure|prompt` for capture forms.
...
9. Local `run bar` in a **workflow** requires a local workflow or script `bar`. In a **rule**, `run` must resolve to a **script** in the referenced module. Imported `run alias.bar` follows the same kind rules per context.
...
6. **Scripts:** Each top-level script is **emitted as a separate executable file** under `build/scripts/<name>` with `chmod +x` (permission `0o755`). The file content is the shebang line (custom or default `#!/usr/bin/env bash`) followed by the body. **Bash scripts** (no shebang or `#!/usr/bin/env bash`) have their body processed through the transpiler's ref-resolution and variable expansion; **non-bash scripts** (custom shebang like `#!/usr/bin/env node`) pass the body through verbatim. The generated module `.sh` invokes scripts via `"$JAIPH_SCRIPTS/<name>" "$@"` inside a `jaiph::run_step` wrapper. A convenience shim `<name>` is still emitted in the module for non-workflow shell callers. The module preamble exports `JAIPH_SCRIPTS` pointing to the scripts directory relative to the module file. **`jaiph run`** and **`jaiph test`** unset any inherited `JAIPH_SCRIPTS` from the parent environment so each build uses its own scripts directory.
...
8. **Conditional steps, fail, wait, prompt schema, recover, capture:** Brace `if` lowers to bash `if …; then … fi`. **`fail`** → stderr message + `exit 1`. **`wait`** → bash `wait`. **Prompt + `returns`:** `jaiph::prompt_capture_with_schema` with JSON extraction and schema validation (same exit codes as before). **ensure … recover:** bounded retry loop; recover **`$1`** from **`JAIPH_RETURN_VALUE_FILE`** after a failed ensure attempt (script stdout captured for that attempt, including nested scripts; stderr only if merged into stdout by the script). **Assignment capture:** **Rules and workflows** use `JAIPH_RETURN_VALUE_FILE` and **`jaiph::set_return_value`** for **`return "…"`** / **`return "$var"`**. **Scripts** do **not** emit `jaiph::set_return_value`; **`run` to a script** captures **stdout** instead.
