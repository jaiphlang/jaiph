...
- For Jaiph-level composition (`name = ensure rule`, `name = run wf`, `name = fn`), assignment captures only the explicit `return` value set by the called rule/workflow/function. Command stdout (`echo`, `printf`, etc.) goes to `.jaiph/runs` artifact files only.
...
4. **run:** Inside a workflow, `run` must target a workflow reference (`foo` or `alias.foo`), not an arbitrary shell command. Optional args after the REF are forwarded to the workflow (e.g. `run deploy "$env"`). **Stdout forwarding:** `run` supports standard shell redirection and pipelines. When the caller redirects stdout (e.g. `run greet "alice" > out.txt` or `run produce | tr a-z A-Z`), the workflow's stdout is forwarded to the caller's stream *and* written to step artifacts — both destinations receive the output. This also works with background jobs (`run greet "alice" > out.txt &`). When stdout is not redirected, output is captured into artifacts only (the default behavior). Prompt-capture steps (`name = run ...`) are not affected — they continue to capture stdout into the variable as before.
5. **Functions:** Top-level `function` blocks define writable shell functions. They are transpiled to namespaced implementations; the original name remains callable via a shim that forwards to the namespaced wrapper. Functions (like rules and workflows) can use `return "value"` or `return "$var"` to set an explicit return value for assignment capture. `return` with a bare integer (e.g. `return 0`) remains a bash exit code.
...
8. **Assignment capture for any step:** `name = <step>` captures a value into the variable `name`. What is captured depends on the step kind (see [Step Output Contract](#step-output-contract)):
   ...
   - `name = run ref [args...]` — Runs the workflow and captures its explicit `return` value into `name`. Workflow step stdout goes to step artifacts only.
   - `name = fn_name [args...]` — Calls the function and captures its explicit `return` value into `name`. Function stdout goes to step artifacts only.
   - `name = <shell_command>` — Runs the shell command and captures its **full stdout** into `name`. This is raw bash `$(...)` behavior; no `return` mechanism applies.
