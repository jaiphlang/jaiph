...
Workflows, rules, and scripts support **named parameters** in their declarations. Named parameters are syntactic sugar over positional arguments — at runtime, they map to positional arguments (`$1`, `$2`, …).
...
- For workflows and rules, the runtime binds `task="$1"; role_name="$2"` at the top of the body.
...
- **Default values:** `workflow deploy(env, version, dry_run = "false")` uses fallback expansion (`dry_run="${3:-false}"`). Required parameters (no default) must be provided at every call site.
...
10. **`wait`:** A line that is exactly `wait` (optionally surrounded by whitespace) is a Jaiph `wait` step, not a shell fallback — it joins async `run ref &` jobs in the same workflow.
...
17. **Parallel processes and `wait`:** Async **`run ref [args] &`** starts a managed workflow or script in the background; a following **`wait`** step (Jaiph keyword — a line containing only `wait`) joins those jobs. For **concurrent bash** (pipelines, `sleep &`, etc.), put the bash in a **`script`** and call it with **`run`**. The workflow `::impl` still runs under the runtime's artifact capture and `set -e`; managed `run` steps use `tee` when stdout is redirected.
    ...
    **Exit status and `wait`:**
    - Bare bash `wait` (no arguments) waits for all background children and returns **0** regardless of individual child exit statuses — standard Bash. The Jaiph **`wait`** step joins managed async jobs started with `run ref &`.
...
## Build artifacts (`jaiph build`)

`jaiph build` produces Bash scripts for Docker/CI distribution. These artifacts are **not** used by `jaiph run` or `jaiph test`, which interpret the AST directly via the Node workflow runtime. The following describes the build output format:

1. Build emits Bash scripts that source the installed stdlib (`$JAIPH_STDLIB`, default `~/.local/bin/jaiph_stdlib.sh`).
