...
- Agent credentials for any exposed workflow that uses `prompt`. See [Authenticate agent backends](agent-auth.md). Set them on the host. Forward any other host variable a workflow needs with `--env`, using the same rules as `jaiph run`.
