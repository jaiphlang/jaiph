...
- **Shell-native (and polyglot)** — Transpiled output is bash. Use **`script`** blocks for command pipelines, tests, and helpers (bash `return` / `exit` only — not Jaiph `return "…"` or `fail` / `log` / `logerr` / `const` inside those bodies). **Polyglot scripts:** add a custom shebang (e.g. `#!/usr/bin/env node` or `#!/usr/bin/env python3`) as the first body line to write scripts in other languages — Jaiph keyword validation is skipped for non-bash shebangs. Scripts are emitted as separate executable files under `build/scripts/`. Module scope: **`const`** or **`local`** for shared values. Generated scripts default **`JAIPH_LIB`** to `<workspace>/.jaiph/lib` (via `JAIPH_WORKSPACE`) so `source "$JAIPH_LIB/…"` resolves predictably. See [Grammar](docs/grammar.md), [CLI environment variables](docs/cli.md#environment-variables), and `.jaiph/language_redesign_spec.md` for the orchestration vs execution boundary.
