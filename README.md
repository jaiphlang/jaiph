...
- **MCP server** — `jaiph mcp ./tools.jh` serves a file's exported defs as [MCP](https://modelcontextprotocol.io/) tools over stdio, so any MCP client (Claude Code, Cursor) can call tested Jaiph defs as tools ([MCP server in 30 seconds](docs/mcp.md)).
...
- uses: jaiphlang/jaiph/actions/setup-jaiph@v0.14.0
  with:
    version: 0.14.0        # semver, a release tag, or 'nightly'
- run: jaiph --version     # jaiph is now on PATH for later steps
```

Verify: `jaiph --version`. Switch versions: `jaiph use nightly` or `jaiph use 0.14.0`.
