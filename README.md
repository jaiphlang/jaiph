...
- **Shell-native** — Transpiled output is bash. Use **`function`** bodies for command pipelines, tests, and helpers; **`const`** / **`local`** (module-level) for shared values. See [Grammar](docs/grammar.md) and `.jaiph/language_redesign_spec.md` for the orchestration vs execution boundary.
