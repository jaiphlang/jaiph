...
You write **`.jh`**; the compiler emits **bash modules and scripts** (intermediate representation) plus a **`.jaiph.map`** sidecar per module so bash stack traces and **`file.sh:line`** diagnostics can be mapped back to **`.jh`** in the CLI. **`jaiph run`** is **launched from TypeScript** (`workflow-launch`): build, resolve bundled **`jaiph_stdlib.sh`** and **`runtime/kernel/`**, then spawn the generated workflow module directly as the detached leader (no `bash -c` launcher). The module runtime entrypoint handles `__jaiph_workflow` and `__jaiph_dispatch` modes and writes run metadata through `JAIPH_META_FILE`. **Prompt** steps, **managed step subprocesses** (`run` / `ensure` / nested rule and workflow children, plus emitted **`script`** executables), **file-backed inbox** (`kernel/inbox.js`), and **`__JAIPH_EVENT__` / `run_summary.jsonl`** (`kernel/emit.js`) run through the **JS kernel** under `src/runtime/kernel/`. Runtime shell wiring now lives directly in `src/jaiph_stdlib.sh`; there is **no legacy bash-only orchestration path** for normal CLI runs. **`jaiph test`** uses the same stack under **`JAIPH_TEST_MODE`**. User **`script`** bodies remain separate OS processes (bash or another shebang).

**Distribution:** `npm run build` produces `dist/` for `node dist/src/cli.js`. **`npm run build:standalone`** (requires [Bun](https://bun.sh)) emits a **single-file** **`dist/jaiph`** binary via **`bun build --compile`** and copies **`jaiph_stdlib.sh`**, **`runtime/kernel/`**, and **`reporting/public`** into **`dist/`** next to it. **Ship the whole `dist/` layout** (binary + siblings). The CLI itself does not need a separate Node install when you use the compiled binary; the **workflow subprocess** still runs **bash** plus stdlib, and the stdlib invokes **`node`** for **`runtime/kernel/*.js`** — keep **Node.js** and **bash** on **`PATH`** for full runs.
