...
- **Shell-native (and polyglot)** — Use **`script`** blocks for command pipelines, tests, and helpers. Script bodies are **opaque** to the compiler — bash or otherwise. The compiler does not parse script lines as Jaiph steps, reject keywords, or strip quotes. This means embedded `node -e` heredocs, inline Python, `const` assignments in JS, and any other valid shell construct compile without interference. For bash scripts, the emitter applies only lightweight transforms (`return` normalization, spacing, import alias resolution); non-bash scripts emit verbatim. **Script isolation:** scripts execute in a clean process environment — only positional arguments, essential system variables, and Jaiph variables (`JAIPH_LIB`, `JAIPH_SCRIPTS`, `JAIPH_WORKSPACE`) are inherited. Module-scoped `const` / `local` values are visible in rules and workflows but **not** in scripts; pass data as arguments or use shared libraries (`source "$JAIPH_LIB/…"`). **Polyglot scripts:** add a custom shebang (e.g. `#!/usr/bin/env node` or `#!/usr/bin/env python3`) as the first body line to write scripts in other languages. The runtime defaults **`JAIPH_LIB`** to `<workspace>/.jaiph/lib` (via `JAIPH_WORKSPACE`) so `source "$JAIPH_LIB/…"` resolves predictably. See [Grammar](docs/grammar.md), [CLI environment variables](docs/cli.md#environment-variables), and `.jaiph/language_redesign_spec.md` for the orchestration vs execution boundary.
...
Verify: `jaiph --version`. Switch version: `jaiph use nightly` or `jaiph use 0.6.0`.
