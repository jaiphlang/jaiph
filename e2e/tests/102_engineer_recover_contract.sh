...
e2e::section "engineer-style catch streams the CI stdout handle and preserves role arg"

# `failure` binds the failed step's stdout as an OUTPUT HANDLE (see Value types
# in docs/language.md). The recover body streams it into a script with
# `stdin failure -> save()` — the CI log flows on stdin, never argv, so a huge
# log cannot hit ARG_MAX — mirroring the real engineer workflow that reads the
# CI log. A sibling role argument still binds normally.
...
e2e::assert_file_exists "${TEST_DIR}/.jaiph/tmp/ensure_ci_passes.last.log" "recover streams the CI stdout handle"
