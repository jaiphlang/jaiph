...
e2e::section "run ... catch ... (single statement) runs once on failure"
...
e2e::section "run ... catch { stmt; stmt; } (block) runs multiple catch statements once"
...
e2e::section "run without catch exits 1 on failure"
...
# Then
e2e::assert_equals "${exit_fail}" "1" "jaiph run exits 1 when run fails without recover"
e2e::assert_contains "${out_fail}" "Run failed (exit 1) with no error output" "stderr reports run failure with exit code"
e2e::assert_contains "${out_fail}" "inspect run_summary.jsonl and step artifacts under" "stderr points at run artifacts"
e2e::pass "run without recover: exit 1 on failure"

e2e::section "run ... catch { multiline prompt with param } parses and runs"
...
e2e::pass "run ... catch { multiline prompt with param }: parses and runs"
