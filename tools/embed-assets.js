...
// The runtime sandbox image is pinned by manifest digest (finding M-6). The
// digest is a release artifact — only known after the image is built and
// published — so it is baked in from package.json's `runtimeImageDigest` at
// release time. An empty string (the default in a working tree) means "no
// pinned digest yet": digest enforcement for the default official image is
// skipped until the release pipeline fills this in. Operators can pin
// immediately via the JAIPH_DOCKER_IMAGE_DIGEST environment variable.
const runtimeImageDigest =
  typeof pkg.runtimeImageDigest === "string" ? pkg.runtimeImageDigest : "";
const versionFile = [
  "// AUTO-GENERATED by tools/embed-assets.js — do not edit by hand.",
  ...
  "",
  "// Manifest digest ('sha256:<hex>') of the pinned default runtime image, baked",
  "// from package.json's `runtimeImageDigest` at release time. Empty = unpinned.",
  `export const RUNTIME_IMAGE_DIGEST = ${JSON.stringify(runtimeImageDigest)};`,
  "",
].join("\n");
