...
## Hard-refresh docs to current runtime architecture <!-- dev-ready -->

**Goal**  
Align all user/developer docs with the post-rewrite architecture: Node-first orchestration, no user-facing `jaiph build` command (the old transpile-to-sh one), scripts-only prep on default run/test path, and internal-only full build compatibility path.

**Context**

- Recent code changes removed the old `jaiph build` (transpile `.jh` → `.sh`) from CLI entry/usage.
- `jaiph run` and `jaiph test` now default to scripts-only preparation (`buildScripts`) before Node runtime execution.
- `ARCHITECTURE.md` has been updated and is the source of truth; most other docs are stale.

Files that **still reference `jaiph build` (old transpile)** and need updating:
- `README.md`
- `docs/grammar.md`
...
Files that **still reference `emit-test.ts`** (deleted internal):
- `docs/contributing.md`
- `CHANGELOG.md`

Files that **still reference `mock function`** (old keyword, now `mock script`):
- `docs/testing.md`
- `docs/index.html`

**Scope**

1. Audit and update all stale references to CLI commands and runtime path in:
   - `README.md`
   - `docs/cli.md`
   - `docs/getting-started.md`
   - `docs/grammar.md`
   ...
   - `docs/sandboxing.md`
   - `docs/index.html`
   - `CHANGELOG.md` (if wording is now inaccurate)
2. Remove instructions/examples that suggest the old `jaiph build` (transpile) is user-facing.
3. Replace `mock function` references with `mock script` in docs.
4. Remove references to `emit-test.ts` from docs.
5. Explicitly document hard-rewrite policy and unsupported legacy behavior where relevant.
6. Ensure examples only use currently supported commands and syntax.
7. Reference source of truth: `ARCHITECTURE.md`, `src/cli/shared/usage.ts` for CLI commands.

**Acceptance criteria**

- No user-facing doc tells users to run the old `jaiph build` (transpile command).
- Runtime flow in docs matches implemented run/test behavior.
- No references to deleted internals (`emit-test.ts`) remain in docs.
- `mock function` → `mock script` in all docs.
- Command examples in docs are executable against current CLI.
