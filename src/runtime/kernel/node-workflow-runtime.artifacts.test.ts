...
function mainErr(runtime: NodeWorkflowRuntime): string {
  const runDir = runtime.getRunDir();
  const name = readdirSync(runDir).find((f) => f.includes("def__main") && f.endsWith(".err"));
  if (!name) throw new Error(`no main .err in ${runDir}: ${readdirSync(runDir).join(", ")}`);
  ...
  try {
    ...
    writeFileSync(
      ...
      [
        ...
        '    write_catch_received(failure)',
        '    write_catch_arg2(extra)',
        "    mark_ready()",
        "  }",
        "}",
        "",
...
test("NodeWorkflowRuntime: recover handle streams a >1MB failed stdout via `stdin failure -> sink()` without argv/OOM", async () => {
  ...
  try {
    ...
    writeFileSync(
      ...
      [
        ...
        '    stdin failure -> record_binding("captured.txt")',
        "  }",
        "}",
        "",
      ].join("\n"),
    ...
    writeFileSync(join(scriptsDir, "record_binding"), '#!/usr/bin/env bash\ncat > "$1"\n', { mode: 0o755 });
...
test("NodeWorkflowRuntime: nested cross-module run applies callee module config and restores caller scope after", async () => {
  ...
  try {
    ...
    // Scope env is observed through Jaiph `${...}` on the callee's return.
    writeFileSync(
      childJh,
      [
        'config {',
    ...
    writeFileSync(
      ...
      [
        ...
        "}",
        "export def main() {",
        '  logerr "parent_before:${JAIPH_AGENT_MODEL}"',
        "  const seen = child.show()",
        '  logerr "child:${seen}"',
        '  logerr "parent_after:${JAIPH_AGENT_MODEL}"',
        "}",
        "",
      ].join("\n"),
    );
  ...
  try {
    ...
    writeFileSync(
      ...
      [
        ...
        "",
        "export def main() {",
        '  const seen = with_prompt("workflow-model")',
        '  logerr "shell:${seen}"',
        "}",
        "",
      ].join("\n"),
    ...
    assert.equal(mainErr(runtime), "shell:\n");
  ...
  try {
    ...
    writeFileSync(
      ...
      [
        ...
        "}",
        "export def main() {",
        '  logerr "parent_before:${JAIPH_AGENT_BACKEND}"',
        "  const seen = child.main()",
        '  logerr "child:${seen}"',
        '  logerr "parent_after:${JAIPH_AGENT_BACKEND}"',
        "}",
        "",
      ].join("\n"),
    );
  ...
  try {
    ...
    writeFileSync(
      ...
      [
        ...
        '    agent.backend = "claude"',
        "  }",
        '  return "alpha:model=${JAIPH_AGENT_MODEL},backend=${JAIPH_AGENT_BACKEND}"',
        "}",
        "",
        "def beta() {",
        "  config {",
        ...
        "  }",
        '  return "beta:model=${JAIPH_AGENT_MODEL},backend=${JAIPH_AGENT_BACKEND}"',
        "}",
        "",
        "export def main() {",
        "  const a = alpha()",
...
test("NodeWorkflowRuntime: ANTHROPIC_API_KEY value in prompt text is redacted in run summary", async () => {
  ...
  try {
    ...
    assert.ok(summaryText.includes("[REDACTED]"), "summary must contain [REDACTED] in place of credential");
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});
  ...
  try {
    ...
    writeFileSync(
      ...
      [
        ...
        "",
        "export def main() {",
        '  logerr "parent:${JAIPH_AGENT_COMMAND}"',
        "  const seen = child.main()",
        '  logerr "child:${seen}"',
        "}",
        "",
    ...
    const actual1 = mainErr(runtime1);
    ...
    const actual2 = mainErr(runtime2);
  ...
  try {
    ...
    writeFileSync(
      ...
      [
        ...
        "}",
        "export def tw() {",
        '  return "${JAIPH_AGENT_TRUSTED_WORKSPACE}"',
        "}",
        "export def cf() {",
        '  return "${JAIPH_AGENT_CURSOR_FLAGS}"',
        "}",
        "export def rd() {",
        '  return "${JAIPH_RUNS_DIR}"',
        "}",
        "",
      ].join("\n"),
    );
    writeFileSync(
      ...
      [
        ...
        "",
        "export def main() {",
        '  logerr "parent-tw:${JAIPH_AGENT_TRUSTED_WORKSPACE}"',
        '  logerr "parent-cf:${JAIPH_AGENT_CURSOR_FLAGS}"',
        '  logerr "parent-rd:${JAIPH_RUNS_DIR}"',
        "  const tw = child.tw()",
        '  logerr "child-tw:${tw}"',
        "  const cf = child.cf()",
        '  logerr "child-cf:${cf}"',
  ...
  try {
    ...
    writeFileSync(
      ...
      [
        ...
        "}",
        "export def main() {",
        '  logerr "cmd:${JAIPH_AGENT_COMMAND}"',
        "}",
        "",
      ].join("\n"),
    );
    ...
    assert.equal(mainErr(runtime), "cmd:my-custom-agent\n");
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});
