...
import { callDef } from "./workflow-call";
 ...
 * `jaiph mcp` calls (`callDef` → `callDefHost`). A workflow that would
 * otherwise run for 30s is terminated by the timeout, returning an error result
 * — this fails if the arming is removed from `callDefHost`.
 */
test("callDef (serve/mcp host path): a run exceeding JAIPH_RUN_TIMEOUT is terminated", async () => {
  ...
  try {
    ...
    const result = await callDef(gen.state.callEnv, "main", [], randomUUID());
