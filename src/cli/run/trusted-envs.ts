 ...
 *    explicit `--env KEY=VALUE` overriding the host value. The runner inherits
 *    the host env; this map is merged so a declared key with only a host value
 *    is still recorded for the same fail-fast as `--env`.
 */
