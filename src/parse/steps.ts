...
/**
 * Try to parse `run <ref>(args) recover(<bindings>) { ... }` syntax (loop semantics).
...
export function parseRunRecoverStep(
  ...
  if (recoverIdx === -1 && !/ recover$/.test(runBody)) return null;
  ...
  if (!right.startsWith("(")) {
    fail(filePath, 'recover requires explicit bindings: recover(<name>) { ... }', innerNo, recoverCol);
  }
